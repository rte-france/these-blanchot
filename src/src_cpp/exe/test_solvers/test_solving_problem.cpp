...
TEST_CASE("3. A problem is solved and we can get the optimal solution", "[solve]") {
    ...
    SECTION("Loop on the instances") {
        ...
        for (auto const& solver_name : factory.get_solvers_list()) {
            ...
            SECTION("Solving the problem with solver's output solves it and the log is sent to a file out.txt") {
                ...
                // Reset standard cout
                std::cout << "DERNIER MESSAGE AVANT DE PASSER EN MODE SCREEN" << std::endl;
